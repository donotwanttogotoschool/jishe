...
func main() {
	...
	r.GET("/overview", func(c *gin.Context) {
		log.Println("访问总体概览页面")
		c.HTML(http.StatusOK, "overview.html", gin.H{})
	})

	r.GET("/field-analysis", func(c *gin.Context) {
		log.Println("访问领域分析页面")
		c.HTML(http.StatusOK, "field-analysis.html", gin.H{})
	})

	r.GET("/time-analysis", func(c *gin.Context) {
		log.Println("访问时间分析页面")
		c.HTML(http.StatusOK, "time-analysis.html", gin.H{})
	})
